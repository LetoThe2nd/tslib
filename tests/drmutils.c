...
static uint32_t transp_mask = 0xFF000000;
...
static int modeset_prepare(int fd)
{
	...
	if (res->count_connectors > 0)
	{
		...
		if (res->count_connectors > 1)
			fprintf(stderr, "more than one connector detected, ignoring the leftovers\n");
	}
...
int open_framebuffer(void)
{
	...
	if ((card = getenv ("TSLIB_FBDEVICE")) == NULL)
		card = defaultdrmdevice;
	...
	/* perform actual modesetting on found connector+CRTC */
	if (modeset_list)
	{
		modeset_list->saved_crtc = drmModeGetCrtc(fd, modeset_list->crtc);
		ret = drmModeSetCrtc(fd, modeset_list->crtc, modeset_list->fb, 0, 0,
	...
	xres = modeset_list->width;
	yres = modeset_list->height;
	rotation = 0;
	/*
	 * so far we always request 32bpp
	 */
	bytes_per_pixel = 4;
...
static void __pixel_loc(int32_t x, int32_t y, union multiptr *loc)
{
	...
	case 0:
	default:
		loc->p8 = &modeset_list->map[y * modeset_list->stride + x * bytes_per_pixel];
		break;
	}
}
